        ...
        <Modal isOpen={this.state.modal} toggle={this.toggle} className={this.props.className}>
          ...
          <ModalBody>
            <div className="modalDisplays">
                <Card id={"image"} style={{"width":"318px", "padding":"15px"}}>
                    <CardImg top width="100%" src={"image"} alt={"image"} />
                    <CardBlock>
                      <CardTitle>{"image".substring(29,"image".length)}</CardTitle>
                      <Button color="secondary" onClick={this.handleSelect} block>SELECT</Button>
                      <Button color="danger" onClick={this.handleDelete} block>DELETE</Button>
                    </CardBlock>
                </Card>
            </div>
          </ModalBody>
